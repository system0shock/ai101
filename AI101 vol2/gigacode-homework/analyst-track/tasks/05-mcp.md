...
Задача: сузить набор tools сервера `ai101Local` так, чтобы аналитику не предлагался
developer-tool — оставить только `search_credit_policy`.

Критерий готовности: после перезапуска CLI в `/mcp` у сервера `ai101Local`
виден только `search_credit_policy`. Запрос из блока A продолжает работать.

Посмотрите в доках по MCP (ссылка выше) — там описаны фильтры `includeTools`
и `excludeTools` в конфигурации `mcpServers`.

## Подсказка

Никакого собственного MCP-сервера писать не нужно — он уже есть. Меняется
только ваш конфигурационный блок `mcpServers`, не код сервера.
...
Если сервер не стартует, проверьте, что команда
`python3 shared/mcp/local_knowledge_server.py` запускается из корня
`gigacode-homework/` и что в конфиге `cwd` указывает туда же.
