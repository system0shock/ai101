...
Задача: добавить свою custom command `/explain-test-failure`, которая готовит
агента к разбору падающего теста: запускает `mvn test`, читает связанный код,
формулирует гипотезу и предлагает минимальную правку — но не правит код сама.

Критерий готовности: команда видна в `/help`, запуск даёт план диагностики
(`mvn test` → read → гипотеза → правка-предложение), а не сразу переписывает
`DiscountPolicy.java`.

Загляните в доки по commands (ссылка выше) — там описан формат
custom command (markdown + опциональный YAML frontmatter).

## Подсказка

Готовая `.gigacode/commands/summarize-context.md` — самый близкий шаблон.
Frontmatter с `description` формально опционален, но без него команда
будет без описания в `/help`.
