...
Задача: сузить набор tools сервера `ai101Local` так, чтобы разработчику
не предлагался analyst-tool — оставить только `get_discount_policy`.

Критерий готовности: после перезапуска CLI в `/mcp` у сервера `ai101Local`
виден только `get_discount_policy`. Запрос из блока A продолжает работать.
