...
Задача: завести своего read-only subagent `discount-policy-reviewer`, который
проверяет только код политики скидок и наличие тестов на обе стороны границы.

Критерий готовности: запрос «используй discount-policy-reviewer, проверь
DiscountPolicy.java» возвращает комментарий по границе `>= 10` и упоминание
отсутствия тестов на 9 и 11 элементов. Subagent не редактирует файлы.
...
Не пишите agent с нуля. Готовый `.gigacode/agents/code-reviewer.md` — близкий
